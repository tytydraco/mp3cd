...
/// [Uid0007].
class Uid0007 extends Converter {
  /// Creates a new [Uid0007].
  Uid0007({required super.inputFile, required super.outputFile});

  @override
  String get id => 'uid0007';
  ...
  Future<void> video() async {
    ...
    if (await inputFile.hasAudio()) {
      ...
      argBuilder
        ..pair('-f', 'lavfi')
        ..pair('-i', 'anullsrc=channel_layout=mono:sample_rate=8000')
        ..pair('-map', '0:v:0')
        ..pair('-map', '1:a')
        ..pair('-ar:a', 8000)
        ..single('-shortest');
    ...
    argBuilder
      ...
      ..pair(
        '-filter:v',
        'transpose=clock:passthrough=portrait,scale=$size:force_original_aspect_ratio=increase:flags=area,crop=$size,vflip',
      )
